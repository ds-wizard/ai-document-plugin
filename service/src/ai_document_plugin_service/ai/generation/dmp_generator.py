...
def _get_reply_keys_at_level(replies: dict, prefix: str) -> list[str]:
    ...
    if not prefix:
        return [k for k in replies.keys() if "." not in k]
    prefix_dot = prefix + "."
    expected_dots = prefix.count(".") + 1
    return [k for k in replies.keys() if k.startswith(prefix_dot) and k.count(".") == expected_dots]
...
def match_replies_selection(questions, replies, km, depth=0, override_uuids=None) -> tuple[list[dict], bool]:
    ...
    *replies*, then looking up the actual reply value.
    ...
    if len(questions) == 0:
        return [], False
    if is_multianswer_question(get_question_path(next(iter(questions.values())), override_uuids)):
        return handle_multi_replies(questions, replies, km, depth, override_uuids)
    else:
        return handle_single_reply(questions, replies, km, depth, override_uuids)
...
def handle_multi_replies(questions, replies, km, depth, override_uuids):
    ...
    *replies* keys, then recursively calls ``match_replies_selection`` for
    ...
    for i, uuid in enumerate(
            get_wildcard_uuids(get_question_path(next(iter(questions.values())), override_uuids), replies.keys())):
...
def handle_single_reply(questions, replies, km, depth, override_uuids):
    ...
    for item in questions.values():
        question_path = get_question_path(item, override_uuids)
        question_paths_covered.add(question_path)
        res, child_has_answer = _build_single_question_result(
            item=item,
    ...
    if depth >= DEPTH_INCLUDE_ALL_ANSWERS:
        synthetic_items = _collect_neighbouring_reply_items(
            questions=questions,
            replies=replies,
            km=km,
            override_uuids=override_uuids,
...
def _build_single_question_result(item, question_path, replies, km, depth, override_uuids):
    ...
    if is_multianswer_question(question_path):
        raise RuntimeError("There is multianswer question among non-multianswer questions at path",
                           question_path)
    if question_path in replies:
        res['reply'] = parse_answer(replies[question_path]["value"], km)
    children, child_has_answer = match_replies_selection(item['children'], replies, km, depth + 1, override_uuids)
...
def _collect_neighbouring_reply_items(questions, replies, km, override_uuids, question_paths_covered) -> list[dict]:
    ...
    for key in keys_at_level:
        if key in question_paths_covered:
            continue
        synthetic = _build_synthetic_question_item(key, replies, km)
        if synthetic is not None:
            synthetic_items.append(synthetic)
    return synthetic_items


def _build_synthetic_question_item(key: str, replies: dict, km: dict) -> Optional[dict]:
    ...
    question_uuid = key.split(".")[-1]
    if question_uuid not in km_questions:
        logger.debug("Question %s from %s not found in the KM", question_uuid, key)
        return None
...
def llm_section_from_qa(
    ...
    return llm.section_from_qa(prompt=prompt, stats=stats, previously_generated=previously_generated)
...
def _flatten_matched_questions(matches: list[dict], section_key: str) -> list[dict]:
...
def _source_questions_table(rows: list[dict]) -> str:
    ...
    df["question_path"] = (
            '<span style="font-size: 0.65em">' + df["question_path"].astype(str) + "</span>"
    )
...
def _generate_section(
    ...
    """
    key = node["key"]
    heading = _heading(depth, key)

    if node.get("assignments") is not None:
        return _generate_leaf_section(node, heading, replies, km, llm, stats)
...
def _generate_leaf_section(
    ...
    key = node["key"]
    matches, _ = match_replies_selection(node["assignments"], replies, km)
    rows = _flatten_matched_questions(matches, key)
    table = _source_questions_table(rows)
    prompt = construct_chapter_prompt(key, matches)
    content = llm_section_from_qa(llm, prompt, stats) if prompt else "No data"
...
def generate_dmp_markdown(
        assignments_tree: list[dict],
        replies: dict,
        km: dict,
        llm: GenerationLLM | None = None,
        stats: Optional[AssignmentStats] = None,
) -> tuple[str, str, AssignmentStats]:
    ...
    for node in assignments_tree:
        section, debug = _generate_section(
            node, 0, replies, km, llm, stats
        )
        parts.append((section, debug))
    markdown = "\n\n".join([s for s, _ in parts])
    debug_markdown = "\n\n".join([d for _, d in parts])
    return markdown, debug_markdown, stats
...
if __name__ == "__main__":
    ...
    with open(file_paths.assignments_output, mode="r", encoding="utf-8") as f:
        data = json.load(f)
    selection = data["assignments"]
    with open(file_paths.knowledge_model, mode="r", encoding="utf-8") as f:
        dmp = json.load(f)
    replies = dmp["replies"]
    km = dmp["knowledgeModel"]

    stats = AssignmentStats()
    markdown, debug_markdown, stats = generate_dmp_markdown(selection, replies, km, stats=stats)

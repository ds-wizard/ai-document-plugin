...
class SectionFormatter:
    def __init__(self, sections: list[SectionRecord]):
        ...
        self.section_key_to_id = None
        self.section_id_to_key = None

    def create_mappings(self, section_id_generator: SectionIdGenerator, stats: AssignmentStats):
        self.section_key_to_id = section_id_generator.generate_leaf_section_ids(self.leaf_sections, stats)
        # Build reverse mapping: id -> key, resolve collisions by appending _1, _2, ...
        self.section_id_to_key = {}
        for key, sec_id in self.section_key_to_id.items():
            new_id = sec_id
            i = 1
    ...
    def get_original_id(self, section_id):

...
def load_config(
    ...
    with open(configured_prompts_path, "r", encoding="utf-8") as handle:
    ...
    return Config(
        api_key=_expand_env_vars(_get(config, "llm_response_generation", "api_key")),
        ...
        files=FilePaths(
            knowledge_model=_resolve_existing_path(_get_file_path(config, "knowledge_model")),
